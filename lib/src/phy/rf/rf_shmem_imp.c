...
// bytes per sample
#define NUMBYTESPERSAMPLE(x) ((x)*sizeof(cf_t))

// samples per byte
#define NUMSAMPLESPERBYTE(x) ((x)/sizeof(cf_t))

#define RF_SHMEM_NTYPE_NONE  (0)  
#define RF_SHMEM_NTYPE_UE    (1)  
#define RF_SHMEM_NTYPE_ENB   (2)

#define NUMSFPERFRAME 10

static const time_t sf_usec         = 1000;
static const struct timeval tv_zero = {0,0};
static const struct timeval tv_sf   = {0,sf_usec};   // 1 sf
static const struct timeval tv_4sf  = {0,4*sf_usec}; // 4 sf
...
typedef struct {
  ...
  int            m_sob;          // is start of burst
  int            m_eob;          // is end of burst
  uint32_t       m_tx_freq;      // tx frequency 
...
sem_t * sem[NUMSFPERFRAME] = {NULL};  // element r/w sf_bin locks

// msg element bins 1 for each sf (tti)
typedef struct {
  rf_shmem_element_t elements[NUMSFPERFRAME];
} rf_shmem_segment_t;
...
static inline uint32_t get_sf_bin(const struct timeval * tv)
 {
   return (tv_to_usec(tv) / sf_usec) % NUMSFPERFRAME;
 }
...
static int rf_shmem_resample(double msg_srate, 
                             double srate, 
                             cf_t * src, 
                             cf_t * dst,
                             int nsamples,
                             int count)
{
  const double decim_factor = msg_srate / srate;
  ...
  if((int) decim_factor != 1)
   {
       ...
       for(int i = 0, n = 0; i < nsamples; i++)
        {
          ...
          for(int j = 0; j < decim_factor; j++, n++)
           {
             avg += ptr[n]; // XXX need to check memory location overrun
           }
     ...
     RF_SHMEM_CONS("msg_srate %.2f, srate %.2f, nsamples %d, decim_factor %.2f, ret %d, count %d", 
                   msg_srate/1e6, srate/1e6, nsamples, decim_factor, ret, count);
#endif
   }
  else
   {
...
double update_rates(void* h, double srate)
{
   ...
   RF_SHMEM_INFO("reset sample rate %.2f -> %.2f MHz", _state->srate / 1e6, srate / 1e6); 
...
int rf_shmem_recv_with_time_multi(void *h, void **data, uint32_t nsamples, 
                                  bool blocking, time_t *full_secs, double *frac_secs)
 {
   ...
   for(uint32_t channel = 0; channel < _state->nof_channels; ++channel)
    {
      ...
      memset(data[channel], 0x0, NUMBYTESPERSAMPLE(nsamples));
         ...
         if(data[channel] && _state->rx_freq[channel] && _state->active[channel])
          {
            ...
            if(timercmp(&_state->tv_this_tti, &element->meta.m_tv_tx_tti, ==))
             {
               if((element->meta.m_nsamples > 0) && (element->meta.m_tx_freq != _state->rx_freq[channel]))
                {
                  RF_SHMEM_INFO("tx_freq %u != our rx_freq %u", element->meta.m_tx_freq, _state->rx_freq[channel]);
                }
               else
                {
   ...
   // retrun 0 will lockup the rx thread
   return nsamples;
 }
         ...
         if(data[channel] && _state->tx_freq[channel] && _state->active[channel])
          {
            ...
            if((channel == 0) &&
               (element->meta.m_nsamples > 0) && 
               timercmp(&tv_tx_tti, &element->meta.m_tv_tx_tti, !=))
             {
               // clear dl sf_bin 
               memset(element, 0x0, sizeof(*element));
             }
            ...
            if(element->meta.m_nsf == 0)
             {
               ...
               element->meta.m_eob        = is_eob;
               element->meta.m_srate      = _state->srate;
               element->meta.m_seqnum     = _state->tx_seqnum++;
               element->meta.m_nsamples   = nsamples;
               element->meta.m_tv_tx_time = tv_now;
